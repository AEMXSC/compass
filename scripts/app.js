...
import * as ai from './ai.js?v=137';
import { TOOL_AGENT_MAP } from './ai.js?v=137';
...
if (githubTokenBtn && githubTokenInput) {
  githubTokenBtn.addEventListener('click', async () => {
    ...
    if (result.ok) {
      ...
      githubTokenStatus.textContent = `Authenticated as ${result.login} — content editing enabled!`;
...
const TOOL_RENDERERS = {
  ...
  /* ─ Experience Production Agent: Gemini Image Card ─ */
  generate_image_gemini(result) {
    const imageUrl = result.image_url || result.imageUrl || '';
    if (!imageUrl) return null;
    const model = result.model || 'gemini-2.5-flash-image';
    const prompt = result.prompt || '';
    const text = result.text || '';

    const googleG = `<svg width="16" height="16" viewBox="0 0 18 18" xmlns="http://www.w3.org/2000/svg">
    ...
    return `
      <div class="result-card gemini-image-card">
        ...
        ${text ? `<div style="font-size:12px;color:var(--text-secondary);margin-bottom:8px">${escapeHtml(text)}</div>` : ''}
        <div class="gemini-img-actions">
          <a href="${escapeHtml(imageUrl)}" target="_blank" rel="noopener" class="gemini-img-action-btn">Open full size</a>
          <a href="${escapeHtml(imageUrl)}" download class="gemini-img-action-btn">Download</a>
        </div>
        <div class="gsearch-footer" style="margin-top:10px">
          <span class="gsearch-powered">Generated by Google Gemini · Hosted on Cloudflare R2</span>
        </div>
      </div>`;
  },

  /* ─ Discovery Agent: Google Search Card ─ */
  gemini_search(result) {
    // Prefer the full cached result (window._lastGeminiSearch) — Claude only
    // receives a compact summary to keep response times fast, but the card
    // shows the complete answer and sources from the cache.
    const data = window._lastGeminiSearch || result;
    const query = data.query || (data.searchQueries || [])[0] || 'Search';
    const answer = data.answer || result.answer || '';
    const sources = data.sources || [];

    const googleG = `<svg width="18" height="18" viewBox="0 0 18 18" xmlns="http://www.w3.org/2000/svg">
      <path d="M17.64 9.2c0-.637-.057-1.251-.164-1.84H9v3.481h4.844a4.14 4.14 0 0 1-1.796 2.716v2.259h2.908c1.702-1.567 2.684-3.875 2.684-6.615z" fill="#4285F4"/>
      <path d="M9 18c2.43 0 4.467-.806 5.956-2.18l-2.908-2.259c-.806.54-1.837.86-3.048.86-2.344 0-4.328-1.584-5.036-3.711H.957v2.332A8.997 8.997 0 0 0 9 18z" fill="#34A853"/>
      <path d="M3.964 10.71A5.41 5.41 0 0 1 3.682 9c0-.593.102-1.17.282-1.71V4.958H.957A8.996 8.996 0 0 0 0 9c0 1.452.348 2.827.957 4.042l3.007-2.332z" fill="#FBBC05"/>
      <path d="M9 3.58c1.321 0 2.508.454 3.44 1.345l2.582-2.58C13.463.891 11.426 0 9 0A8.997 8.997 0 0 0 .957 4.958L3.964 7.29C4.672 5.163 6.656 3.58 9 3.58z" fill="#EA4335"/>
    ...
    const sourcesHtml = sources.length ? `
      <div class="gsearch-sources">
        <div class="gsearch-sources-label">Sources</div>
        ${sources.slice(0, 5).map((s) => {
          const uri = s.uri || '';
          const domain = uri ? (() => { try { return new URL(uri).hostname.replace('www.', ''); } catch { return uri; } })() : '';
          const favicon = domain ? `https://www.google.com/s2/favicons?domain=${domain}&sz=16` : '';
          const title = escapeHtml(s.title || domain || uri);
          return `<a href="${escapeHtml(uri)}" target="_blank" rel="noopener" class="gsearch-source-chip">
            ${favicon ? `<img src="${favicon}" width="12" height="12" style="border-radius:2px;flex-shrink:0" onerror="this.style.display='none'">` : ''}
            <span class="gsearch-source-title">${title}</span>
            ${domain ? `<span class="gsearch-source-domain">${escapeHtml(domain)}</span>` : ''}
          </a>`;
        }).join('')}
      </div>` : '';

    const answerHtml = answer ? `<div class="gsearch-answer">${md(answer)}</div>` : '';

    return `
      <div class="result-card gsearch-card">
        <div class="gsearch-header">
          <span class="gsearch-logo">${googleG}</span>
          <span class="gsearch-title">Google Search</span>
          <span class="gsearch-query-chip">${escapeHtml(query)}</span>
        </div>
        ${answerHtml}
        ${sourcesHtml}
        <div class="gsearch-footer">
          <span class="gsearch-powered">Grounded by Google Search · Synthesized by Gemini 2.5 Flash</span>
        </div>
      </div>`;
  },
...
async function validateAndStoreGitHubToken(token) {
  try {
    ...
    if (!resp.ok) return { ok: false, error: resp.status === 401 ? 'Token invalid or expired' : `GitHub returned ${resp.status}` };
...
function storeComplianceMarkdown(fullText) {
  if (/\b(compliance|governance|accessibility|WCAG|SEO|brand|audit|checks?\s+passed)\b/i.test(fullText) &&
      fullText.length > 300) {
